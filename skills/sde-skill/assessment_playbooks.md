...
3. Fetch the in-scope domain YAML file(s) directly from `${{data_url}}`:
   - **Safe Access & Identity**: `${{data_url}}safe_access_identity.yaml`
   - **Safe Data Management**: `${{data_url}}safe_data_management.yaml`
   - **Safe Governance & Compliance**: `${{data_url}}safe_governance_compliance.yaml`
   - **Safe Outputs & Disclosure Control**: `${{data_url}}safe_outputs_disclosure_control.yaml`
   - **Safe Projects & Operations**: `${{data_url}}safe_projects_operations.yaml`
   - **Safe Technology & Engineering**: `${{data_url}}safe_technology_engineering.yaml`
