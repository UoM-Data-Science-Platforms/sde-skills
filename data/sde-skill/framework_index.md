...
- Safe Access & Identity (`safe-access-identity`): `https://sdertp.org/data/safe_access_identity.yaml`
- Safe Data Management (`safe-data-management`): `https://sdertp.org/data/safe_data_management.yaml`
- Safe Governance & Compliance (`safe-governance-compliance`): `https://sdertp.org/data/safe_governance_compliance.yaml`
- Safe Outputs & Disclosure Control (`safe-outputs-disclosure-control`): `https://sdertp.org/data/safe_outputs_disclosure_control.yaml`
- Safe Projects & Operations (`safe-projects-operations`): `https://sdertp.org/data/safe_projects_operations.yaml`
- Safe Technology & Engineering (`safe-technology-engineering`): `https://sdertp.org/data/safe_technology_engineering.yaml`

Each competency defines entry, mid, and senior level skill statements.

6 domains / 21 subdomains / 76 competencies. Levels everywhere: entry, mid, senior.
...
Detail YAML URL: `https://sdertp.org/data/safe_access_identity.yaml`
...
- **Role-Based Access Control** (`role-based-access-control`): Implements access permissions based on organisational roles within secure data environments.
...
Detail YAML URL: `https://sdertp.org/data/safe_data_management.yaml`
...
Detail YAML URL: `https://sdertp.org/data/safe_governance_compliance.yaml`
...
- **Consent Management** (`consent-management`): Manages processes for obtaining, recording, and honouring data subject consent.
...
Detail YAML URL: `https://sdertp.org/data/safe_outputs_disclosure_control.yaml`
...
Deploys and administers workflow engines, data airlocks, and secure code repositories to ensure consistency, reliability, and traceability of output checking processes.

- **Workflow Engines** (`workflow-engines`): Deploys and administers workflow engines to ensure consistency, reliability, and traceability of output checking processes.
- **Data Airlocks** (`data-airlocks`): Installs and administers data airlock applications, defines role-based access levels required for operational tasks (e.g.
- **Code Repositories** (`code-repositories`): Installs and configures code repository systems (e.g.
...
Establishes protocols, investigation procedures, and immediate containment measures when an accidental disclosure or unauthorised output export is suspected or identified.

- **Accidental Disclosure Management** (`accidental-disclosure-management`): Detects, assesses, and handles accidental disclosure events to mitigate confidentiality risks and prevent unauthorised dissemination of sensitive information.

### Emergency Response (`emergency-response`)

Prepares and executes rapid response procedures for suspected data breaches, data leaks, and critical security incidents in research environments.

- **Emergency Response Protocols** (`emergency-response-protocols`): Coordinates emergency response actions, forensics acquisition, and cross-team containment during confirmed or suspected data breaches.
...
Detail YAML URL: `https://sdertp.org/data/safe_projects_operations.yaml`
...
- **Research Project Facilitation** (`research-project-facilitation`): Supports researchers in utilising secure data environments effectively.
...
Detail YAML URL: `https://sdertp.org/data/safe_technology_engineering.yaml`
